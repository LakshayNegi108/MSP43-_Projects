...
void ssd1306_printText_2(uint8_t x, uint8_t y, char *ptString) {
    ssd1306_setPosition(x, y);
    ...
    while (*ptString != '\0') {
        ...
        uint8_t i;
        for(i = 0; i< 5; i++) {
            buffer[i+1] = font_2[*ptString - 32][i];
        }
...
void ssd1306_printText_2_wide(uint8_t x, uint8_t y, char *ptString) {
    ssd1306_setPosition(x, y);
...
//        }
        ...
        uint8_t i;
        for(i = 0; i< 7; i++) {
            buffer[i+1] = font_2_wide[*ptString - 32][i];
        }

        buffer[8] = 0x0;
        __delay_cycles(2000);
        i2c_write(SSD1306_I2C_ADDRESS, buffer, 9);
        __delay_cycles(2000);
        ptString++;
        x+=8;
    }
...
void ssd1306_printText_matra(uint8_t x, uint8_t y, char *ptString) {
    ssd1306_setPosition(x, y);
    uint8_t pos;
    switch (*ptString) {
        case 'A':
...
//        }
        ...
        uint8_t i;
        for(i = 0; i< 3; i++) {
            buffer[i+1] = font_matra[pos][i];
        }

        buffer[4] = 0x0;
        __delay_cycles(2000);
        i2c_write(SSD1306_I2C_ADDRESS, buffer, 5);
        __delay_cycles(2000);
        ptString++;
        x+=4;
    }
...
void dim(uint8_t dim_display) {
  uint8_t contrast;

  switch(dim_display){
      case 0:   contrast = 0; break;// Dimmed display
      case 1:   contrast = 0x9F; break;
      case 2:   contrast = 0xCF; break;
  }
...
void ssd1306_printText_on_line(uint8_t x, uint8_t y, char *ptString) {
    ssd1306_setPosition(x, y);
