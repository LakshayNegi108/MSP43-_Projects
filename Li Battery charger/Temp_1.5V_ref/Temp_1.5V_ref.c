...
float R_t = 6400.0; //Known Resistance of Thermistor

void register_settings_for_ADC10()
{
    ADC10AE0 |= AIN;                            // P1.0 ADC option select
    ADC10CTL1 = INCH_0;                         // ADC Channel -> 1 (P1.0)
    ADC10CTL0 = SREF_1 + REFON + ADC10SHT_3 + ADC10ON; // Ref -> Vcc, 64 CLK S&H , ADC - ON
}
...
void main(void)
{
    ...
    while (1)
    {
        ...
        adc_value = (ADC10MEM) * (1.50) / (1023.00); // mapping 10-bit conversion result of ADC to corresponding voltage
        ...
        printf("Value: %d.%d, ", int_part, decimal_part);
